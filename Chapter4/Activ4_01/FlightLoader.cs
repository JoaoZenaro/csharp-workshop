    ...
    class FlightLoader : IFlightLoader
    {
        ...
        public IList<Flight> Import(string filePath)
        {
                ...
                .Where(ln => !string.IsNullOrWhiteSpace(ln)))
            {
                ...
                if (fields.Length < ImportFieldIndex.Carrier || string.IsNullOrEmpty(fields[ImportFieldIndex.Agency]))
                    continue;

                float.TryParse(fields[ImportFieldIndex.PaidFare], out float paidFare);
