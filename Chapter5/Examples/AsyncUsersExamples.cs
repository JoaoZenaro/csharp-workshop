...
namespace Chapter5.Examples
{
    ...
    public class AccountGenerator
    {
        public async Task CreateAccounts()
        {
            ...
            var accountCreationTasks = users.Select(
                user => user.Region == RegionName.North ? Task.Run(() => CreateNorthernAccount(user)) : Task.Run(() => CreateOtherAccount(user))
                ).ToList();
    ...
    public static class AsyncUsersExamples
    {
